...
![Demo: browsing a category breakdown, opening the Train sub-category breakdown, reassigning a transaction's category live, switching months, and toggling dark mode](docs/demo.gif)

*(Demo uses fabricated data, not real statements — your own data never leaves your machine.)*
...
- **`expense_analyser/api.py`** + **`expense_analyser/static/`** — FastAPI backend and web frontend. `GET /api/months` (expenditure + separate-category totals per month), `GET /api/categories` (every known category — the fixed list plus anything ever actually assigned, for the reassignment dropdown), `GET /api/months/{month}/categories` (chart-only breakdown — excludes `Income` and `SEPARATE_CATEGORIES` like Stony Brook, since a huge irregular payment next to routine categories makes a chart unreadable; both stay fully visible in the KPI tiles, the CLI reports, and `review_category`), `GET /api/months/{month}/categories/{group}` (a `CATEGORY_GROUPS` group's real members for that month, e.g. Subway/Amtrak/LIRR under "Train" — empty for an ordinary category), `GET /api/months/{month}/transactions`, and `PATCH /api/transactions/{transaction_id}` (updates category + merchant rule, same as the CLI — wired into the UI's reassignment dropdown). Amounts are returned as raw integer cents, not dollars — formatting is a display concern for whatever consumes the API. The frontend (`static/index.html`/`app.js`/`style.css`) is vanilla JS, no framework or charting library — a month sidebar, KPI tiles (total spend, transaction count, top category), a category list with colorblind-safe icon badges and percentage bars (color is never the sole way a value is shown), a group breakdown showing each real sub-category's net total (so a refund that outweighs its original charge stays visible rather than silently netting to a small, confusing number), and a transaction table where clicking a category pill reassigns it live via the API.
