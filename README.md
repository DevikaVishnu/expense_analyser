...
- **`expense_analyser/api.py`** + **`expense_analyser/static/`** — FastAPI backend and web frontend. `GET /api/months` (expenditure + separate-category totals per month), `GET /api/months/{month}/categories` (chart-only breakdown — excludes `Income` and `SEPARATE_CATEGORIES` like Stony Brook, since a huge irregular payment next to routine categories makes a chart unreadable; both stay fully visible in the hero figure, the CLI reports, and `review_category`), `GET /api/months/{month}/categories/{group}` (a `CATEGORY_GROUPS` group's real members for that month — empty for an ordinary category, so the frontend can use "got nothing back" to mean "not a group, no sub-chart to show"), `GET /api/months/{month}/transactions`, and `PATCH /api/transactions/{transaction_id}` (updates category + merchant rule, same as the CLI, built but not yet wired into the UI — view-only for this first pass). Amounts are returned as raw integer cents, not dollars — formatting is a display concern for whatever consumes the API. The frontend (`static/index.html`/`app.js`/`style.css`) is vanilla JS with hand-rolled SVG bar charts (no charting library) — a monthly expenditure trend, a per-month category breakdown (click a group bar like "Train" for a sub-breakdown chart of its real members), and a transaction table, wired together by clicking a month.
